...
async def create_artwork(
    ...
    if artwork.secret_threshold < 0:
        raise HTTPException(
            status_code=400, detail="Secret threshold must be non-negative"
        )
    ...
    if len(artwork.title) > 200:
        raise HTTPException(
            status_code=400, detail="Title cannot exceed 200 characters"
        )

    # Validate description length if provided
    if artwork.description and len(artwork.description) > 2000:
        raise HTTPException(
            status_code=400, detail="Description cannot exceed 2000 characters"
        )
...
async def update_artwork(
    ...
    if (
        ...
        and len(update_data["description"]) > 2000
    ):
        raise HTTPException(
            status_code=400, detail="Description cannot exceed 2000 characters"
        )
